...
const ViewUser = () => {
  ...
  useEffect(() => {
    const fetchPhotos = async () => {
      try {
        const response = await axios.get(`https://localhost:7095/api/Users/GetUserPhotos/${id}`);
      ...
      {photos.length > 0 && (
        <div>
          {photos.map((photo, index) => (
            <img key={index} src={`data:image/jpeg;base64,${photos[index].image}`} alt={`Photo ${index + 1}`}  style={{ maxWidth: "400px", marginTop: "10px"}}/>
          ))}
        </div>
      )}
    </div>
